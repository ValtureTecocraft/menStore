...
}: // onDelete
{
  ...
  //   useEffect(() => {
  //     const storedToggle = localStorage.getItem(`toggle_${
  //         // item
  //     }`);
  //     if (storedToggle) {
  ...
  return (
    <div
      className={`p-5 text-xl cursor-move border-b flex gap-3 justify-between items-center ${
        isDarkMode
    ...
    >
      <div
        ...
        className={`md:w-9 w-8 h-[26px] md:h-8 cursor-pointer flex justify-center items-center rounded-full ${
      ...
      <p
        className={`ml-2 pt-2 w-full ${toggle && "line-through text-gray-500"}`}
      >
        {item}
      </p>
      {isHovered && (
